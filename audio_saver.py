...
class SaveAudioAdvanced:
    ...
    def save_audio(self, audio, filename_prefix, format,
                   ...
                   show_debug=False, prompt=None, extra_pnginfo=None):
        ...
        if isinstance(audio, (tuple, list)) and len(audio) == 2 and isinstance(audio[0], (torch.Tensor, np.ndarray)) and isinstance(audio[1], int):
            ...
            if show_debug: print("[SaveAudioAdvanced] Info: Detected standard AUDIO tuple format (Tensor, int).")
        # Style 2: Wrapped dictionary tuple ( {'waveform':..., 'sample_rate':...}, )
            ...
            if 'waveform' in audio_dict and 'sample_rate' in audio_dict and isinstance(audio_dict.get('waveform'), (torch.Tensor, np.ndarray)) and isinstance(audio_dict.get('sample_rate'), int):
                ...
                if show_debug: print("[SaveAudioAdvanced] Info: Detected wrapped dictionary AUDIO format ({'waveform':..., 'sample_rate':...}, ).")
        # Style 3: Plain dictionary { 'waveform':..., 'sample_rate':... }
            ...
            if 'waveform' in audio and 'sample_rate' in audio and isinstance(audio.get('waveform'), (torch.Tensor, np.ndarray)) and isinstance(audio.get('sample_rate'), int):
                ...
                if show_debug: print("[SaveAudioAdvanced] Info: Detected plain dictionary AUDIO format {'waveform':..., 'sample_rate':...}.")
        ...
        if show_debug: print(f"[SaveAudioAdvanced] Input format validated successfully. Sample Rate: {sample_rate_input}")
        ...
        if show_debug: print(f"[SaveAudioAdvanced] Waveform is now a tensor. Initial dtype: {waveform.dtype}")

        if waveform.numel() == 0: print("[SaveAudioAdvanced] Error: Input waveform tensor is empty."); return {}
        if waveform.device != torch.device('cpu'):
            if show_debug: print("[SaveAudioAdvanced] Moving waveform to CPU.")
            waveform = waveform.cpu()

        # --- Shape Handling ---
        if show_debug: print(f"[SaveAudioAdvanced] Waveform shape before final processing: {waveform.shape}")
        ...
        if show_debug: print(f"[SaveAudioAdvanced] Waveform shape for saving: {waveform.shape}")
        ...
        try:
            ...
            if show_debug: print(f"[SaveAudioAdvanced] Calculated save path: {filepath}")
        ...
        if format == 'wav':
            ...
            if wav_encoding not in encoding_map:
        ...
        # --- MP3 ---
            ...
            if show_debug: print(f"[SaveAudioAdvanced] Info: Using MP3 bitrate: {mp3_bitrate} kbps")
            # MP3 Data prep
            if not torch.is_floating_point(waveform):
                 if show_debug: print("[SaveAudioAdvanced] Warning: Input waveform is not float for MP3 save. Normalizing.")
                 try:
                     dtype_info = torch.iinfo(waveform.dtype); max_val = dtype_info.max; min_val = dtype_info.min
                     if min_val < 0: waveform = waveform.float() / max(abs(max_val), abs(min_val))
                     else: waveform = (waveform.float() / max_val) * 2.0 - 1.0
        ...
        try:
            ...
            if show_debug: print("[SaveAudioAdvanced] torchaudio.save() completed without raising an exception.")

            if os.path.exists(filepath):
                 if show_debug: print(f"[SaveAudioAdvanced] File check: Confirmed file exists at {filepath}")
                 saved = True
            else:
        ...
        # --- Result for UI ---
        if saved:
            if show_debug: print(f"[SaveAudioAdvanced] Operation successful. Reporting saved file to UI.")
