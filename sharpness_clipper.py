...
def calculate_sharpness(image):
    ...
    Args:
        image: torch.Tensor or numpy array of shape (H, W, C)

    Returns:
        float: Sharpness score (variance of Laplacian)
    """
...
class SharpestFrameClipper:
    ...
    RETURN_NAMES = ("clipped_images", "sharpest_frame_index", "sharpness_score")
    ...
    def clip_to_sharpest(self, images, last_n_frames, skip_text_frames, skip_black_white_frames, black_white_threshold, show_debug):
        ...
        sharpest_frame_index = frame_info[sharpest_idx_in_window][0]
        best_sharpness = float(sharpness_scores[sharpest_idx_in_window])

        if show_debug:
            print(f"[Clip to Sharpest Frame] Sharpest frame found at index {sharpest_frame_index} (sharpness: {best_sharpness:.2f})")
        ...
        return (clipped_images, sharpest_frame_index, best_sharpness)
